...
class ParserHelper:
    ...
    def get_parser(self, *, command) -> ArgumentParser:
    ...
    def parse(self, *, args: str, command: str) -> Optional[Namespace]:
        """Parses an argument string for a given command."""

        # WORKAROUND: exit_on_error is not honored ...
        try:
            return self.get_parser(command=command).parse_args(args=shlex.split(args))
